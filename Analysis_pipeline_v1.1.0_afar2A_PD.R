...
#scenarios <- read_xlsx("config/model_inputs_snnpr2.xlsx",sheet="Scenarios")
...
#ServiceCategory Colors
sc_tritanopia_colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#9467bd", "#8c564b",  "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#aec7e8",  "#ffbb78", "#98df8a", "#c5b0d5", "#ff9896", "#ff9896", "#9edae5")
scats <- c("Pregnancy","Sexual health","Sick child","Immunization","Nutrition","Tuberculosis","Malaria","First Aid","NCDs","Mental health","NTDs","HIV","Family planning","FI", "Adult Outpatient")
names(sc_tritanopia_colors) <- scats
sc_fillScale <- scale_fill_manual(name = "ServiceCat",values = sc_tritanopia_colors)
sc_colorScale <- scale_color_manual(name = "ServiceCat",values = sc_tritanopia_colors)

#ServiceLabel Colors
sl_tritanopia_colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#9467bd", "#8c564b",  "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#aec7e8",  "#ffbb78", "#98df8a", "#c5b0d5", "#ff9896", "#ff9896", "#9edae5")
slabs <- c("Pregnancy","Sexual health","IMNCI","RI","Nutri","Tuberculosis","Malaria","First Aid","NCDs","Mental health","NTDs","HIV","FP","FI", "Adult Outpatient")
names(sl_tritanopia_colors) <- slabs
sl_fillScale <- scale_fill_manual(name = "ServiceLabel",values = sl_tritanopia_colors)

#ClinicalCategory Colors
cc_tritanopia_colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#9467bd", "#8c564b",  "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#aec7e8")
ccats <- c("Non-health","Supportive","Clinical - Acute","Clinical - Chronic","Clinical - Preventative","Non-clinical","Clinical - Promotive","Surveillance","Overhead")
names(cc_tritanopia_colors) <- ccats
cc_fillScale <- scale_fill_manual(name = "Category",values = cc_tritanopia_colors)
cc_colorScale <- scale_color_manual(name = "Category",values = cc_tritanopia_colors)
...
#Colour code for different categories
my_palette <- viridis(13, option = "D", direction = -1, begin = 0, end = 1, alpha = 1)
...
ggplot() +
  ...
  labs(x="Year", y="Hours per Week per Catchment Pop") + labs(title = paste("Time Allocation by Service Category"))
...
ServiceCat_Clinical <- Mean_ServiceCat %>%
  subset(ClinicalOrNon=="Clinical" & ServiceCat!="") %>%
  filter(Year >= 2021 & Year <= 2035) %>% 
